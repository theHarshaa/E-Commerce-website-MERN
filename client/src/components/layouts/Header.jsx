...
<div className="items-baseline shadow-lg shadow-white-100/50 py-0 px-4 navbar">
      <Link className="text-4xl hover:no-underline brand flex items-center" to='/'>
      🛍️ Shopping 
      </Link>
      <nav>
        <section className="m-3 MOBILE-MENU flex lg:hidden">
          ...
          <div className={isNavOpen ? "showMenuNav" : "hideMenuNav"}> 
            ...
            <ul className="MENU-LINK-MOBILE-OPEN flex flex-col items-center justify-evenly min-h-[250px]">
              <li className="text-base px-1 py-2 text-slate-700 rounded-lg hover:font-bold my-8 uppercase">
            ...
            !auth.user ? (
            <>
            <li className="text-base p-1 text-slate-700 rounded-lg hover:font-bold my-8 uppercase">
          ...
          <li className="text-base p-1 text-slate-700 rounded-lg hover:font-bold my-8 uppercase">
            <NavLink to="/login" className="text-3xl hover:no-underline">Login</NavLink>
              ...
              <li  className="text-base p-1 text-tahiti text-slate-700 rounded-lg  my-8 uppercase">
              <div className="dropdown">
                ...
                <li>
                  <NavLink  to={`/dashboard/${auth?.user?.role === 1 ? "admin" : "user" }`} className="text-3xl hover:no-underline text-base p-1 text-slate-700 rounded-lg hover:font-bold my-8 uppercase" role="menuitem" tabIndex={-1} id="menu-item-0">Dashboard</NavLink>
                </li>
                <li><hr className="dropdown-divider" /></li>
                <li>
                  <NavLink onClick={handleLogout} to="/login" className="text-3xl hover:no-underline text-base p-1 text-slate-700 rounded-lg hover:font-bold my-8 uppercase" role="menuitem" tabIndex={-1} id="menu-item-1">Logout</NavLink>
                </li>
                </ul>
              </div>
              </li>
        ...
        <SearchInput />
          ...
          <li className="nav-item dropdown text-base p-1 text-slate-700 rounded-lg hover:font-bold  my-8 uppercase">
 ...
 {categories?.map((c) => (
  <li><Link className="dropdown-item" to={`/category/${c.slug}`}>{c.name}</Link></li>
          ...
          <li className="text-base p-1 text-slate-700 rounded-lg hover:font-bold my-8 uppercase">
            <NavLink to="/login" className="hover:no-underline text-lg">Login</NavLink>
              ...
              <li  className="text-base p-1 text-tahiti text-slate-700 rounded-lg hover:font-bold my-8 uppercase">
              <div className="dropdown">
  ...
  <ul className="dropdown-menu">
  <li>
    <NavLink  to={`/dashboard/${auth?.user?.role === 1 ? "admin" : "user" }`} className="hover:no-underline text-lg p-1 text-slate-700 rounded-lg hover:font-bold my-8 uppercase" role="menuitem" tabIndex={-1} id="menu-item-0">Dashboard</NavLink>
    </li>
    <li><hr className="dropdown-divider" /></li>
    <li>
  <NavLink onClick={handleLogout} to="/login" className="hover:no-underline text-lg p-1 text-slate-700 rounded-lg hover:font-bold my-8 uppercase" role="menuitem" tabIndex={-1} id="menu-item-1">Logout</NavLink>
  </li>
  </ul>
</div>
              </li>
